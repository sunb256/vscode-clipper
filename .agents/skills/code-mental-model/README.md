...
- 概要・各図・実装の構成・補足を独立ノードに分けた自己完結型Obsidian Canvasファイル
...
MarkdownとCanvasは同じ内容を持ちますが、CanvasはMarkdownを参照しません。Canvasでは概要、各Mermaid図、実装の構成、補足を別々のテキストノードへ格納し、実装の全項目は一つのノード内にまとめます。ノードを文書順に連結するとMarkdown全文を再現でき、`.md`と`.canvas`のどちらも単独でコピーして利用できます。

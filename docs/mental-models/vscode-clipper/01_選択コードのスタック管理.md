...
sequenceDiagram
    ...
    Clipper->>WinAPI: AutoHotkey経由でClipboardを読み出す
    WinAPI-->>Clipper: HTMLとテキスト
...
- **クリップボード形式を取得する**
  - VS Codeのコピー結果をAutoHotkeyのWindows API呼び出し経由でHTMLとプレーンテキストの両形式として読み出す。
  ...
  - [`helper/vscode-clipper.ahk`](vscode://sunb256.vscode-clipper/open?repo=vscode-clipper&path=helper%2Fvscode-clipper.ahk&line=117) — `ReadClipboard`, `ReadClipboardValue`
